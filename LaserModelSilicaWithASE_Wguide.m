%Laser model that includes ASE
%End goal: simuate W waveguid so that ASE wavelengths have a much larger
%mode and leak out of the core.
%current status: some parts (up to line 170) have been change to have
%wavelength dependent w but then realized that all the equations have been
%derived based on the assumption that w is wavelength independent so
%stopped until further assessment of equations. 
...
lambdaP = 1030e-9;
lambdaL = 1050e-9;
lambdaS = [1035e-9:dlam:lambdaL-dlam lambdaL lambdaL+dlam:dlam:1150e-9];%[lambdaL:dlam:1150e-9];%[1035e-9:dlam:lambdaL-dlam lambdaL lambdaL+dlam:dlam:1150e-9]; %%MAYBE CHANGE THIS -- to make spacing even
freqP = c/lambdaP;
freqS = c./lambdaS;
...
V = 2.44;
w = NaN(1,length(lambdaS));
for i = 1:length(lambdaS)
    if lambdaS(i) > lambdaL + 15e-9
        w(i) = core_radius*(0.65+1.619/V^1.5+2.879/V^6);
    else
         w(i) = core_radius*0.65;
    end
end
...
QmaxNoLoss = -h*(tauRad/tau*freqP-freqF_SE)/(cs_aP+cs_eP)*Am(1)*cs_aP*N0/tauRad*(-2*core_radius^2/w^2);

Popt = IsatP*pi/2*(-2*core_radius^2./log(1-etta(1))).*(-1*(2-etta(1))+sqrt(etta(1).^2+4*(1-etta(1))*cs_aP*N0/loss_ba*(tau/tauRad*lambdaP/lambdaF_SE-1)))/2./(1-etta(1));

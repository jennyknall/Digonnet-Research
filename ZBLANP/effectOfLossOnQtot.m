...
loss_b = lossdB/etta*log(10)/10; %m^-1
...
Pp0 = zeros(1,length(loss_b)); %stores optimal Pp0 for each loss_b
Ptotsignal = zeros(1,length(loss_b)); %stores total signal power emitted from fiber for each Pp0
Pabs = zeros(1,length(loss_b)); %stores values for pump power absorbed for each loss_b
dQ_dt1 = zeros(1,length(loss_b)); %stores values for dQ/dt for each loss_b
dQz_dt2 = zeros(length(loss_b),length(z)); %stores dQ/dt as a function of z for each loss_b
dQz_dtSE = zeros(length(loss_b),length(z)); %stores the SE contribution to dQ/dt as a function of z for each loss_b
dQzcorr_dtSE = zeros(length(loss_b),length(z)); %stores the correction term the SE cooling as a funciton of z for each loss_b
dQz_dtPhononLoss = zeros(length(loss_b),length(z)); %stores the phonon loss contribution to dQ/dt as a function of z for each loss_b
dQz_dtScat = zeros(length(loss_b),length(z)); %stores the scattering loss contribution to dQ/dt as a function of z for each loss_b
dSF_dz = zeros(length(loss_b),length(z)); %stores the forward signal contribution to dQ/dt as a function of z for each loss_b
dSB_dz = zeros(length(loss_b),length(z)); %stores the backward signal contribution to dQ/dt as a function of z for each loss_b 
dP_dz = zeros(length(loss_b),length(z));%stores the Pump abs contribution to dQ/dt as a function of z for each loss_b
dQ_dt2 = zeros(1,length(loss_b)); %stores values for dQ/dt for each loss_b
dQ_dtSE = zeros(1,length(loss_b)); %stores the SE contribution to dQ/dt for each loss_b
dQ_dtSEcorr = zeros(1,length(loss_b)); %stores the SE correction term to dQ/dt for each loss_b
dQ_dtPhononLoss = zeros(1,length(loss_b)); %stores the Phonon loss contribution to dQ/dt for each loss_b
dQ_dtScat = zeros(1,length(loss_b)); %stores the scattering loss contribution to dQ/dt for each loss_b
dQ_dtSF = zeros(1,length(loss_b)); %stores the forward signal contribution to dQ/dt for each loss_b
dQ_dtSB = zeros(1,length(loss_b)); %stores the backward signal contribution to dQ/dt for each loss_b
dQ_dtP = zeros(1,length(loss_b)); %stores the pump abs contribution to dQ/dt for each loss_b
maxdQz_dt2 = zeros(2,length(loss_b)); %stores the coordinates of the maximum value of dQz_dt2
                                    %first row: max values
                                    %second row: index of z where max value is
lambdaF_SL = zeros(1,length(loss_b)); %stores the mean flouresnce wavelength for each pump power calculated from the 
                                    %populations of each sublevel.
dTz = zeros(length(loss_b),length(z));  %stores the change in temp at a location z along the fiber
dTmax = zeros(1,length(loss_b)); %stores the max temperature change along the fiber in air
dTzVac = zeros(length(loss_b),length(z));  %stores the change in temp at a location z along the fiber in a vaccuum
dTavg = zeros(1,length(loss_b)); %stores the average change in temperature across the fiber 
dTVacAvg = zeros(1,length(loss_b)); %stores the average  temperature across the fiber in a vaccuum. 
dTVacMin = zeros(1,length(loss_b)); %stores the min temperature across the fiber in a vaccuum. 

for p = 1:length(loss_b)
...
dPp_dz = @(z,pump,Psf,Psb,vw) pump*cs_aP*N0/comTerm2(pump,Psf,Psb,vw)...
    ...
    *((exp(-2*core_radius^2/w^2)-1)-(1+tauRatio)*log((tauRatio+1+comTerm2(pump,Psf,Psb,vw)*exp(-2*core_radius^2/w^2))/(tauRatio+1+comTerm2(pump,Psf,Psb,vw)))/comTerm2(pump,Psf,Psb,vw))...
    -loss_b(p)*pump*(1-exp(-2*core_radius^2/w^2));
...
dPsforward_dz = @(z,pump,Psf,Psb,vw) -Psf.*(cs_eS(vw)+cs_aS(vw))*N0*comTerm1(pump,Psf,Psb,vw)/comTerm2(pump,Psf,Psb,vw)*((exp(-2*core_radius^2/w^2)-1)...
    ...
    *((exp(-2*core_radius^2/w^2)-1)-(1+tauRatio)*log((tauRatio+1+comTerm2(pump,Psf,Psb,vw)*exp(-2*core_radius^2/w^2))/(tauRatio+1+comTerm2(pump,Psf,Psb,vw)))/comTerm2(pump,Psf,Psb,vw))...
    -loss_b(p)*Psf*(1-exp(-2*core_radius^2/w^2));
...
dPsbackward_dz = @(z,pump,Psf,Psb,vw) Psb.*(cs_eS(vw)+cs_aS(vw))*N0*comTerm1(pump,Psf,Psb,vw)/comTerm2(pump,Psf,Psb,vw)*((exp(-2*core_radius^2/w^2)-1)...
    ...
    *((exp(-2*core_radius^2/w^2)-1)-(1+tauRatio)*log((tauRatio+1+comTerm2(pump,Psf,Psb,vw)*exp(-2*core_radius^2/w^2))/(tauRatio+1+comTerm2(pump,Psf,Psb,vw)))/comTerm2(pump,Psf,Psb,vw))...
    +loss_b(p)*Psb*(1-exp(-2*core_radius^2/w^2));


%find pump power such that the fiber has zero net temp change
noTempChange = @(pump) N0(p)*PsatP/PsatPa*log((tauRatio+1+pump/PsatP*exp(-2*core_radius^2/w^2))/(tauRatio+1+pump/PsatP))*(Am*h*freqF_SE/tauRad-(cs_aP+cs_eP)*PsatP*(1+tauRatio))...
    +pump*etta*(loss_ba+N0(p)*cs_aP-N0(p)*PsatP/PsatPa*(cs_aP+cs_eP));

Popt(p) = IsatP*pi/2*(-2*core_radius^2/log(1-etta))*(-1*(2-etta)+sqrt(etta^2+4*(1-etta)*cs_aP*N0(p)/loss_ba(p)*(tau/tauRad*lambdaP/lambdaF_SE-1)))/2/(1-etta);

if Popt(p) > 0
    Pp0(p) = fzero(noTempChange,Popt(p)*10);
    if isnan(Pp0(p))
        Pp0(p) = fzero(noTempChange,Popt(p)*20);
    end
else
    Pp0(p) = 0;
end
...
for zz = 1:length(z)
    ...
    dQz_dtPhononLoss(p,zz) = (Pp(zz)+sum(Psforward(:,zz)+Psbackward(:,zz)))*loss_ba(p)*(1-exp(-2*core_radius^2/w^2));
    
    dQz_dtScat(p,zz) = (Pp(zz)+sum(Psforward(:,zz)+Psbackward(:,zz)))*loss_bs(p)*(1-exp(-2*core_radius^2/w^2));
...
% grid on
% hold on
% plot(z,N2z/N0,'DisplayName',sprintf('%g dB/m',lossdB(p)));
...
grid on 
hold on
plot(z,dQz_dt2(p,:),'DisplayName',sprintf('%g dB/m',lossdB(p)));
xlabel('z (m)');
ylabel('dQ/dt (J/s)');
...
% hold on
% grid on
% plot(z,dTz(p,:),'DisplayName',sprintf('%g dB/m',lossdB(p)));
% xlabel('z (m)');
% ylabel('dT (K)');
...
% hold on
% grid on
% plot(z,dTzVac(p,:)-rTemp,'DisplayName',sprintf('%g dB/m',lossdB(p)));
% xlabel('z (m)');
% ylabel('dT (K)');
...
% hold on
% grid on
% plot(tempInVac_t.x,tempInVac_t.y,'DisplayName',sprintf('%g dB/m',lossdB(p)));
% xlabel('t (s)');
...
%dQ/dt(z) vs. absorptive loss
figure(10)
hold on
loglog(lossdB,dQz_dt2(:,1)*1e3);
xlabel('Absorptive loss (dB/m)');
ylabel('Extracted heat per unit length (mW/m)');
box on
pbaspect([1.256 1 1])
%grid on

%Qtot vs. absorptive loss
figure(11)
hold on
plot(lossdB,dQ_dt2*1e3);
xlabel('Absorptive loss (dB/m)');
ylabel('Maximum total extracted heat (mW)');
box on

% %Maximum T in Vac vs. loss
% figure(16)
% hold on
% plot(lossdB,dTVacMin-rTemp)
% xlabel('Fiber Loss (dB/m)')
% ylabel('Maximum Tempurature Change (K)')
% title('Maximum Tempurature Change in a Vacuum vs. Fiber Loss')
% grid on

% %Maximum T in Air vs. loss
% figure(17)
% hold on
% plot(lossdB,dTmax)
% xlabel('Fiber Loss (dB/m)')
% ylabel('Maximum Tempurature Change (K)')
% title('Maximum Tempurature Change in Air vs. Fiber Loss')
% grid on
...
% conc = 0.5e26:.1e26:3e26;
% maxLoss = etta*cs_aP*N0*(tau/tauRad*freqF_SE/freqP-1)*10/log(10)
% 
% figure(18)
% plot(conc,maxLoss)
% xlabel('Concentration (m^{-3})');
% ylabel('Maximum Allowable Absorptive Loss (dB/m)');
% title('Maximum Allowable Absorptive Loss vs. Yb^{3+} Concentration');
% grid on


% NPL(1) = interp1(dQz_dt2(:,1),lossdB,dQz_dt2(1,1)*0.9)
% conc(1) = N0_wtPercent
% radlifetime(1)= tauRad
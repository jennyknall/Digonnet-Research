...
lambdaP = 990e-9:2e-9:1030e-9;%975e-9:3e-9:1030e-9;%980e-9:2e-9:1050e-9;%860e-9:10e-9:1050e-9;
lambdaS = 990e-9:dlam:1100e-9;%[850e-9:dlam:1100e-9]; round(lambdaF_SE*1e9)/1e9
lambdaGains = [992 1010 1020]*1e-9;%[round(lambdaF_SE*1e9)/1e9 1010e-9 1020e-9];
...
%cross sectional a6eas for a given wavelength
...
gainAtStart = zeros(length(lambdaP),length(lambdaS)); %stores the gain as the start of the fiber for each wavelength
...
while iterations <= maxIterations
    ...
    if iterations > maxIterations && tries == maxTries
        break
    end
...
% grid on 
% hold on
% plot(z,dQz_dt2(l,:),'DisplayName',sprintf('%g nm',lambdaP(l)*1e9));
% xlabel('fiber length (m)');
% ylabel('dQ/dt (J/s)');
...
figure(10)
hold on
plot(lambdaP*1e9,avgdQz_dt2(1,:)*1e3);
xlabel('Pump wavelength (nm)');
ylabel('avergae heat extracted per unit length (mW/m)');
...
hold on
yyaxis left
plot(lambdaP*1e9,gainAtStart(:,indexGain))
ylabel('Gain coefficient (m^{-1})');
yyaxis right
plot(lambdaP*1e9,avgdQz_dt2(1,:)*1e3);
ylabel('Average heat extracted per unit length (mW/m)');
xlabel('Pump wavelength (nm)');
legend(sprintf('%g nm',lambdaGains(1)*1e9),sprintf('%g nm',lambdaGains(2)*1e9),sprintf('%g nm',lambdaGains(3)*1e9),'dQ/dt')
grid on

% %gain of select wavelengths vs. pump wavelength
% %dQ/dt(z) vs. Pump wavelength (uses the avg value of dQ/dt)
% figure(13)
% yaxis = plotyy(lambdaP*1e9,gainAtStart(:,indexGain),lambdaP*1e9,avgdQz_dt2(1,:)*1e3);

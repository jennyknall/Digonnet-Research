...
N0_wtPercent = [0.00005:.005:.1 .1:.1:6];
...
Nc = 6.47e27; %m^-3
tauNonRad = 2*pi/9*tauRad*(Nc/N0(p))^2;%67.6e-3;%211.65e-3;%1e10;%
...
Popt(p) = IsatP*pi/2*(-2*core_radius^2/log(1-etta))*(-1*(2-etta)+sqrt(etta^2+4*(1-etta)*cs_aP*N0(p)/loss_ba*(tau/tauRad*lambdaP/lambdaF_SE-1)))/2/(1-etta);
...
hold on
box on
plot(N0_wtPercent, dQ_dt2*1e3);
xlabel('Concentration (wt% Yb)')
ylabel('Maximum total heat extraction (mW)')
pbaspect([2 1 1])

figure(13)
hold on
box on
plot(N0_wtPercent, dQ_dt2./Pp0*100);
xlabel('Concentration (wt% Yb)')

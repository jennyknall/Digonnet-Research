...
N0_wtPercent = [0.00005:.00005:.001 .001:.005:.1 .1:.01:3.3];
...
maxN0 = Nc*sqrt(2*pi/9*(lambdaP/lambdaF_SE-1))/2.42e26 %assuming no background absorption loss
...
Pp0(p) = IsatP*pi/2*(-2*core_radius^2./log(1-etta)).*(-1*(2-etta)+sqrt(etta.^2+4*(1-etta)*cs_aP*N0(p)/loss_ba*(tau/tauRad*lambdaP/lambdaF_SE-1)))/2./(1-etta);
...
figure(12)
hold on
plot(N0_wtPercent,maxdQz_dt2(1,:)*1e3)
xlabel('Yb concentration (wt% Yb)');
ylabel('Max heat extraction per unit length (mw/m)');
%ylim([min(maxdQz_dt2(1,:)*1e3) 0])

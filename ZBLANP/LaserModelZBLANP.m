...
lambdaP = 1015e-9; %pump wavelength
lambdaS = 1025e-9; %laser wavelength
freqP = c/lambdaP;
freqS = c/lambdaS;
...
grid on 
hold on
plot(z,dQz_dt2(p,:)*1e3,'DisplayName',sprintf('%g mW',Pp0(p)*1e3));
xlabel('z (m)');
ylabel('dQ/dt (mW/m)');
...
% grid on 
% hold on
% plot(zNew(1:segment)/zNew(segment),dQ_looped,'DisplayName',sprintf('%g mW',Pp0(p)*1e3));

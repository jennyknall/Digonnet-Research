...
len = .04; %length of cavity (m)
dz = len/100;
Pp0 = 380; %pump power (W)
loops = 8; %humber of loops for heat distribution equilization. allowed values: 1,2,3,4,6,7,9,13
...
h_looped = 40; %W/m^2/K
segment = floor(length(z)/loops);
dQ_looped = zeros(1,segment);
for i = 1:loops
    seg = (i-1)*segment+1:i*segment;
    dQ_looped = dQ_looped+dQz_dt2(seg);
end
% lastBit = loops*segment+1:length(dQz_dt2);
% zeroarray = zeros(1,segment-length(lastBit));
% dQ_looped = dQ_looped+[dQz_dt2(lastBit) zeroarray];

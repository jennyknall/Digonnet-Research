...
len = 3;
dz = len/300;
Pp0 = 300;%.001:0.01:.6;%[.2 .175 .15 .124];%[0.001:0.01:0.7];%.05:0.05:1;%[.09:.03:2 3];
%Psforward0 = 0;
...
lambdaP = 1030e-9;
lambdaL = 1050e-9;
lambdaS = lambdaL;%[1035e-9:dlam:lambdaL-dlam lambdaL lambdaL+dlam:dlam:1150e-9]; %%MAYBE CHANGE THIS -- to make spacing even
...
validWavelengths = find(ones(1,length(lambdaS)));
wavelengthsASE = find(validWavelengths ~= indexL);
...
%     if abs(errorf(iterations)) < maxErrorL || iterations > maxIterations
%         break
%     else
...
%     end
    ...
    if (sum(abs(Psforward(validWavelengths,1)) > maxErrorS) == 0 && abs(Pp(1)-Pp0(p)) < maxErrorS)...

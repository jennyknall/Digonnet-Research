...
%signal. Automatically converges. Does NOT approximate intensity in
%fiber as P(z)/Area
...
% %N2(z) ***Approximate! uses I = P/A
N2 = @(Pp,Ps) (Pp*f/IsatPa+Ps*f/IsatSa)./(1+Pp*f/IsatP+Ps*f/IsatS)*N0;
%signal gain coefficients ***Approximate! uses I = P/A
...
comTerm1 = @(Pp,Psf,Psb) (Pp/PsatPa+(Psf+Psb)/IsatSa/Am);
comTerm2 = @(Pp,Psf,Psb) (Pp/PsatP+(Psf+Psb)/IsatS/Am);
...
%postion.
dPp_dz = @(z,Pp,Psf,Psb) Pp*cs_aP*N0/comTerm2(Pp,Psf,Psb)...
    *log((1+comTerm2(Pp,Psf,Psb)*exp(-2*core_radius^2/w^2))/(1+comTerm2(Pp,Psf,Psb)))...
    +Pp*N0*(cs_aP*(Psf+Psb)/IsatS/Am-(cs_aP+cs_eP)*(Psf+Psb)/IsatSa/Am)/comTerm2(Pp,Psf,Psb)...
    *((exp(-2*core_radius^2/w^2)-1)-log((1+comTerm2(Pp,Psf,Psb)*exp(-2*core_radius^2/w^2))/(1+comTerm2(Pp,Psf,Psb)))/comTerm2(Pp,Psf,Psb));
%forward signal power
dPsforward_dz = @(z,Pp,Psf,Psb) -Psf*(cs_eS+cs_aS)*N0*comTerm1(Pp,Psf,Psb)/comTerm2(Pp,Psf,Psb)*((exp(-2*core_radius^2/w^2)-1)...
    -log((1+comTerm2(Pp,Psf,Psb)*exp(-2*core_radius^2/w^2))/(1+comTerm2(Pp,Psf,Psb)))/comTerm2(Pp,Psf,Psb))...
    -Psf*cs_aS*N0*(1-exp(-2*core_radius^2/w^2))-2*h*freqS*dvS*cs_eS*N0*comTerm1(Pp,Psf,Psb)/comTerm2(Pp,Psf,Psb)...
    *((exp(-2*core_radius^2/w^2)-1)-log((1+comTerm2(Pp,Psf,Psb)*exp(-2*core_radius^2/w^2))/(1+comTerm2(Pp,Psf,Psb)))/comTerm2(Pp,Psf,Psb));
%backward signal power
dPsbackward_dz = @(z,Pp,Psf,Psb) Psb*(cs_eS+cs_aS)*N0*comTerm1(Pp,Psf,Psb)/comTerm2(Pp,Psf,Psb)*((exp(-2*core_radius^2/w^2)-1)...
    -log((1+comTerm2(Pp,Psf,Psb)*exp(-2*core_radius^2/w^2))/(1+comTerm2(Pp,Psf,Psb)))/comTerm2(Pp,Psf,Psb))...
    +Psb*cs_aS*N0*(1-exp(-2*core_radius^2/w^2))+2*h*freqS*dvS*cs_eS*N0*comTerm1(Pp,Psf,Psb)/comTerm2(Pp,Psf,Psb)...
    *((exp(-2*core_radius^2/w^2)-1)-log((1+comTerm2(Pp,Psf,Psb)*exp(-2*core_radius^2/w^2))/(1+comTerm2(Pp,Psf,Psb)))/comTerm2(Pp,Psf,Psb));

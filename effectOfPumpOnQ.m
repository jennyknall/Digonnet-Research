...
Qtot = zeros(1,length(Pp0)); %stores values for dQ/dt for each Pp0
...
dTVacMin = zeros(1,length(Pp0)); %stores the minimum temperature of fiber in vaccuum
...
for r = 0:dr:core_radius-dr %takes the average gain across radius of fiber
    for i = 1:length(z)
        ...
        gainPump(1,i) = gainPump(:,i)+gammaS(Pp(i),Psforward(:,i).'+Psbackward(:,i).',r,indexP).'/numPoints;
    end
end
for i = 1:length(lambdaS)
...
% title('Change in Temperature vs Initial Pump Power');
%legend('Pabs-Ps','differential equation');
% 
...
% figure(15)
% hold on
% plot(Pp0,dTVacMin-300)
% xlabel('Pp0 (W)')
% ylabel('Minumum Temperature (K)')
% title('Minimum Temperature in Vacuum vs. Pp0')
% grid on
% 
% %min T in air vs. Pp0
% figure(16)
...
% title('Maximum Temperature Change in Air vs. Pump Power')
% grid on

% %Qtot vs. Pp0 - log scale
% figure(17)
% %hold on
% grid on
% semilogy(Pp0,Qtot)
% xlabel('Pump Power (W)')
% ylabel('Total heat extracted (W)')
% title('Total heat extracted vs. Pump Power')

%Qtot vs. Pp0 - linear scale
figure(18)
hold on
grid on
plot(Pp0,Qtot)
...
% hold on
% grid on
% plot(Pp0,Qtot./Pp0*100)
% xlabel('Input pump power (W)')
% ylabel('Cooling efficiency (%)')
% title('Cooling efficiency vs. Pump Power')
% 
% %cooling efficiency AND Qtot vs. Pp0
% figure(19)
% hold on
% yyaxis left
% plot(Pp0,Qtot*1e3)
...
% xlabel('Input pump power (W)')
% ylabel('Cooling efficiency (%)')
% title('Cooling efficiency and total heat extracted vs. Pump Power')
% grid on

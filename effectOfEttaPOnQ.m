...
Pp0 = zeros(1,length(ettaP)); %stores the optimum power for each ettaP
modeRad = zeros(1,length(ettaP)); %stores the effective radius of the pump for each ettaP
Ptotsignal = zeros(1,length(ettaP)); %stores total signal power emitted from fiber for each ettaP
Pabs = zeros(1,length(ettaP)); %stores values for pump power absorbed for each ettaP
dQ_dt1 = zeros(1,length(ettaP)); %stores values for dQ/dt for each ettaP
dQz_dt2 = zeros(length(ettaP),length(z)); %stores dQ/dt as a function of z for each ettaP
dQz_dtSE = zeros(length(ettaP),length(z)); %stores the SE contribution to dQ/dt as a function of z for each ettaP
dQzcorr_dtSE = zeros(length(ettaP),length(z)); %stores the correction term the SE cooling as a funciton of z for each Pp0
dQz_dtPhononLoss = zeros(length(ettaP),length(z)); %stores the phonon loss contribution to dQ/dt as a function of z for each ettaP
dQz_dtScat = zeros(length(ettaP),length(z)); %stores the scattering loss contribution to dQ/dt as a function of z for each ettaP
dSF_dz = zeros(length(ettaP),length(z)); %stores the forward signal contribution to dQ/dt as a function of z for each ettaP
dSB_dz = zeros(length(ettaP),length(z)); %stores the backward signal contribution to dQ/dt as a function of z for each ettaP 
dP_dz = zeros(length(ettaP),length(z));%stores the Pump abs contribution to dQ/dt as a function of z for each ettaP
dQ_dt2 = zeros(1,length(ettaP)); %stores values for dQ/dt for each ettaP
dQ_dtSE = zeros(1,length(ettaP)); %stores the SE contribution to dQ/dt for each ettaP
dQ_dtSEcorr = zeros(1,length(ettaP)); %stores the SE correction term to dQ/dt for each Pp0
dQ_dtPhononLoss = zeros(1,length(ettaP)); %stores the Phonon loss contribution to dQ/dt for each ettaP
dQ_dtScat = zeros(1,length(ettaP)); %stores the scattering loss contribution to dQ/dt for each ettaP
dQ_dtSF = zeros(1,length(ettaP)); %stores the forward signal contribution to dQ/dt for each ettaP
dQ_dtSB = zeros(1,length(ettaP)); %stores the backward signal contribution to dQ/dt for each ettaP
dQ_dtP = zeros(1,length(ettaP)); %stores the pump abs contribution to dQ/dt for each ettaP
avgGain = zeros(length(ettaP),length(lambdaS)); %stores the average gain across the fiber for each wavelength
maxGain = zeros(length(ettaP),length(lambdaS)); %stores the maximum gain across the fiber for each wavelength
maxdQz_dt2 = zeros(2,length(ettaP)); %stores the coordinates of the maximum value of dQz_dt2
                                    %first row: max values
                                    %second row: index of z where max value is
lambdaF_SL = zeros(1,length(ettaP)); %stores the mean flouresnce wavelength for each pump power calculated from the 
                                    %populations of each sublevel.
lambdaFF_ASE = zeros(1,length(ettaP)); %stores the mean flouresence wavelength of the ASE for each pump wavelength
lambdaFB_ASE = zeros(1,length(ettaP));
dTz = zeros(length(ettaP),length(z));  %stores the change in temp at a location z along the fiber
dTavg = zeros(1,length(ettaP)); %stores the average change in temperature across the fiber 
dTmin = zeros(1,length(ettaP)); %stores the minimum change in temperature across the fiber (most cooling if negative)

for l = 1:length(ettaP)

modeRad(l) = sqrt(-2*core_radius^2/log(1-ettaP(l)));  
w = modeRad(l);
Am = pi*w^2/2;
...
Pp0(l) = IsatP*pi/2*(-2*core_radius^2/log(1-ettaP(l)))*(-1*(2-ettaP(l))+sqrt(ettaP(l)^2+4*(1-ettaP(l))*cs_aP*N0/loss_ba*(tau/tauRad*lambdaP/lambdaF_SE-1)))/2/(1-ettaP(l));
...
grid on
hold on
plot(z/max(z),Pp,'DisplayName',sprintf('ettaP = %g',ettaP(l)));
...
grid on
hold on
plot(z,N2z/N0,'DisplayName',sprintf('ettaP = %g',ettaP(l)));
...
grid on 
hold on
plot(z,dQz_dt2(l,:),'DisplayName',sprintf('ettaP = %g',ettaP(l)));
xlabel('fiber length (m)');
ylabel('dQ/dt (W/m)');
...
hold on
grid on
plot(z,dTz(l,:),'DisplayName',sprintf('ettaP = %g',ettaP(l)));
xlabel('fiber length (m)');
ylabel('dT (K)');
...
title('Temperature Change vs. ettaP');
grid on

%dQ/dt(z) vs. ettaP
figure(10)
plot(ettaP,maxdQz_dt2(1,:));
xlabel('ettaP');
ylabel('dQ/dt (W/m)');
title('dQ/dt vs. ettaP');
grid on
...
%modeRad vs. ettaP
figure(12)
plot(ettaP,modeRad*1e6)
xlabel('ettaP');
ylabel('mode Radius (um)');
title('mode Radius vs. ettaP');
grid on

...
N0_wtPercent = 6; %Yb203
N0 = N0_wtPercent/0.0271*1.81e26;  %m^-3
f = 1/Area;
gamma = 1;
...
Popt = IsatP*pi/2*(-2*core_radius^2./log(1-Nup)).*(-1*(2-Nup)+sqrt(Nup.^2+4*(1-Nup)*cs_aP*N0/loss_ba(p)*(tau/tauRad*lambdaP/lambdaF_SE-1)))/2./(1-Nup);
if Popt > 0
    Pp0(p) = Popt;
else
    Pp0(p) = 0;
...
figure(10)
hold on
plot(lossdB,dQz_dt2(:,1));
xlabel('Absorptive loss (dB/m)');
ylabel('Heat Extracted (W/m)');
title('dQ/dt vs. absorptive loss');
box on
%grid on

%Pp0 vs. absorptive loss
figure(11)
hold on
plot(lossdB,Pp0);
xlabel('Absorptive loss (dB/m)');
ylabel('launched pump (W)');
title(';aunched pump vs. absorptive loss');
box on
%grid on
...
maxLoss = etta*cs_aP*conc*(tau/tauRad*freqF_SE/freqP-1)*10/log(10)

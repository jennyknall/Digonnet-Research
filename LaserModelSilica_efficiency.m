...
len = 5.4; %length of cavity (m)
dz = len/400;
Pp0 = 50;%40:20:60;%[.8:.2:2 1000:300:2000];%.2:.1:1;%.2:.1:.5;%.1:.1:.5;%0.06:0.02:.20;%:0.1:1;%.01:.05:1;%[.09:.03:1 2 3]; %pump power (W)
loops = 2; %humber of loops for heat distribution equilization. allowed values: 1,2,3,4,6,7,9,13

maxIterations = 500; %convergence iterations
maxError = 1e-4; 
errorStep = 0.005; 
...
efficiencyPin = zeros(1,length(Pp0)); %stores PSout/Pp0 for each Pp0
efficiencyPabs = zeros(1,length(Pp0)); %stores PSout/Pabs for each Pp0
...
efficiencyPin(p) = PSout(p)/Pp0(p);
efficiencyPabs(p) = PSout(p)/Pabs(p);
...
% grid on
% 
%output signal power AND total heat generation vs. Pp0
figure(11)
hold on
yyaxis left
plot(Pp0,PSout)
...
Pin = [5.3 6 7 8 12 16 25 35 45 55 70 90 150 200 300]; 
Pout = [2.5 2.9 3.5 3.9 6.0 8 12.4 17.5 22.5 27.5 35.2 45.3 76.6 104.1 153.4];
eff = Pout./Pin;
lengthCav = [0.35 0.45 0.55 0.68 1.12 1.58 2.6 3.72 4.85 5.97 7.62 9.85 16.3 21.36 32.79];

%efficiency vs. PSout
figure(12)
hold on
plot(PSout,efficiencyPin*100);
plot(PSout,efficiencyPabs*100);
plot(Pout,eff*100);
xlabel('Laser output power (W)');
ylabel('Efficiency (%)');
legend('Pout/Pin','Pout/Pabs','Pout/Pin for dQ=0');
